...
export default function Camera({ initialPhotoUrl, onCapture }) {
  ...
  try {
    let media;
    try {
      media = await navigator.mediaDevices.getUserMedia(pickConstraints(true));
    } catch {
      media = await navigator.mediaDevices.getUserMedia(pickConstraints(false));
    }
    setStream(media);
  } catch (e) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      fd.append("patientName", "John Doe");
      fd.append("note", "Captured via webcam");

      await axios.post("/api/patient/register", fd);
      alert("Uploaded!");
    } catch (e) {
      console.error(e);
      alert("Upload failed");
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4">
        <div className="relative">
          ...
          <div className="w-full h-[220px] rounded-2xl border border-gray-300 bg-white overflow-hidden">
      ...
      {!stream && !photo && (
        <button
          type="button"
          onClick={startCamera}
          className="w-full h-[50px] rounded-[10px] bg-[#6F3CDB] text-white font-medium text-[16px] "
        >
          Start Camera
        </button>
      )}
     ...
     {previewSrc && <div>
      ...
      {/* Optional fallback input for devices without camera */}
